...
export const purchaseService = {
    ...
    async getSuppliers(params?: Record<string, string>) {
        return apiClient.get<{ results: Supplier[] }>('/purchase/suppliers/', params);
    },

    async getSupplier(id: number) {
        return apiClient.get<Supplier>(`/purchase/suppliers/${id}/`);
    },
    ...
    async createSupplier(data: Partial<Supplier>) {
        return apiClient.post<Supplier>('/purchase/suppliers/', data);
    },

    async updateSupplier(id: number, data: Partial<Supplier>) {
        return apiClient.patch<Supplier>(`/purchase/suppliers/${id}/`, data);
    },
    ...
    async getMaterialRequests(params?: Record<string, string>) {
        return apiClient.get<{ results: MaterialRequest[] }>('/purchase/material-requests/', params);
    },

    async getMaterialRequest(id: number) {
        return apiClient.get<MaterialRequest>(`/purchase/material-requests/${id}/`);
    },

    async createMaterialRequest(data: Partial<MaterialRequest> & { items: MaterialRequestItem[] }) {
        return apiClient.post<MaterialRequest>('/purchase/material-requests/', data);
    },

    async updateMaterialRequest(id: number, data: Partial<MaterialRequest> & { items?: MaterialRequestItem[] }) {
        return apiClient.patch<MaterialRequest>(`/purchase/material-requests/${id}/`, data);
    },

    async deleteMaterialRequest(id: number) {
        return apiClient.delete(`/purchase/material-requests/${id}/`);
    },

    async submitMaterialRequest(id: number) {
        return apiClient.post(`/purchase/material-requests/${id}/submit/`, {});
    },

    async approveMaterialRequest(id: number) {
        return apiClient.post(`/purchase/material-requests/${id}/approve/`, {});
    },

    async rejectMaterialRequest(id: number, reason: string) {
        return apiClient.post(`/purchase/material-requests/${id}/reject/`, { reason });
    },
    ...
    async overrideMaterialRequestStatus(id: number, request_status: string, notes?: string) {
        return apiClient.post<MaterialRequest>(`/purchase/material-requests/${id}/override_status/`, { request_status, notes });
    },

    // ===== Purchase Orders =====
    async getPurchaseOrders(params?: Record<string, string>) {
        return apiClient.get<{ results: PurchaseOrder[] }>('/purchase/purchase-orders/', params);
    },

    async getPurchaseOrder(id: number) {
        return apiClient.get<PurchaseOrder>(`/purchase/purchase-orders/${id}/`);
    },

    async createPurchaseOrder(data: any) {
        return apiClient.post<PurchaseOrder>('/purchase/purchase-orders/', data);
    },
    ...
    async createDirectPurchaseOrder(data: {
        ...
        return apiClient.post<PurchaseOrder>('/purchase/purchase-orders/create-direct/', data);
    },
    ...
    async overridePurchaseOrderStatus(id: number, data: { receipt_status?: string; payment_status?: string; notes?: string }) {
        return apiClient.post<PurchaseOrder>(`/purchase/purchase-orders/${id}/override_status/`, data);
    },

    // Aggregated PO expense report (totals by supplier / date / status)
    async getPurchaseExpenseReport(params?: Record<string, string>) {
        return apiClient.get<PurchaseExpenseReport>('/purchase/purchase-orders/expense_report/', params);
    },

    async submitPurchaseOrder(id: number) {
        return apiClient.post(`/purchase/purchase-orders/${id}/submit/`, {});
    },

    async approvePurchaseOrder(id: number, notes?: string) {
        return apiClient.post(`/purchase/purchase-orders/${id}/approve/`, { notes });
    },

    async sendToAccounts(id: number) {
        return apiClient.post(`/purchase/material-requests/${id}/send_to_accounts/`, {});
    },
    ...
    async generatePOs(id: number, data: { order_date: string; delivery_date?: string; tax_type?: string; is_tax_inclusive: boolean; items?: any[] }) {
        return apiClient.post<{ message: string; po_numbers: string[]; skipped_items?: string[] }>(`/purchase/material-requests/${id}/generate_purchase_orders/`, data);
    },

    async markReceived(id: number) {
        return apiClient.post(`/purchase/material-requests/${id}/mark_received/`, {});
    },

    async markStocked(id: number) {
        return apiClient.post(`/purchase/material-requests/${id}/mark_stocked/`, {});
    },

    async markPoPaid(id: number, payment_reference: string) {
        return apiClient.post(`/purchase/purchase-orders/${id}/mark_paid/`, { payment_reference });
    },

    async closePurchaseOrder(id: number) {
        return apiClient.post(`/purchase/purchase-orders/${id}/close_order/`, {});
    },

    // Warehouse Receiving Workflow
    async markShipmentArrived(id: number) {
        return apiClient.post(`/purchase/purchase-orders/${id}/mark_shipment_arrived/`, {});
    },

    async markPoStocked(id: number) {
        return apiClient.post(`/purchase/purchase-orders/${id}/mark_stocked/`, {});
    },

    async markPartiallyReceived(id: number, items: Array<{ id: number; newly_received: number }>) {
        return apiClient.post(`/purchase/purchase-orders/${id}/mark_partially_received/`, { items });
    },

    async markMissingReceived(id: number) {
        return apiClient.post(`/purchase/purchase-orders/${id}/mark_missing_received/`, {});
    },

    async downloadMRPDF(id: number) {
        const token = tokenManager.getAccessToken();
        const response = await fetch(`${API_BASE_URL}/purchase/material-requests/${id}/download_pdf/`, {
            headers: {
    ...
    async downloadPOPDF(id: number) {
        const token = tokenManager.getAccessToken();
        const response = await fetch(`${API_BASE_URL}/purchase/purchase-orders/${id}/download_pdf/`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
